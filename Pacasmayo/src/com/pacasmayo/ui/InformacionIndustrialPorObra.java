...
public class InformacionIndustrialPorObra extends MainScreen implements ListFieldCallback {
    ...
    public InformacionIndustrialPorObra(CanalIndustrial ci, long desde, long hasta, Obra obra) {
    	this.obra = obra;
    	Inicializar(ci,desde,hasta);
    }
    
    public InformacionIndustrialPorObra(CanalIndustrial ci, long desde, long hasta) {
    	Inicializar(ci,desde,hasta);
    }
    
    private void Inicializar(CanalIndustrial ci, long desde, long hasta){
    	canalIndustrial = ci;
    	fDesde = desde;
    	fHasta = hasta;
	...
	private void buscar() {
		...
		result = informaciones.getObjetosByIdClienteFechasObra(canalIndustrial.getCodigo(), canalIndustrial.getFecha(),obra);
    	if ( lstTomas != null ) {
    ...
    MenuItem mnIngresar = new MenuItem("Ingresar", 110, 10) {
        public void run() {
        	...
        	refresca = true;
    		Estilos.pushScreen(new IngresoDatosCI(canalIndustrial, null,obra));
        }
    };
    ...
    MenuItem mnModificar = new MenuItem("Modificar", 110, 10) {
        public void run() {
        	...
        	if( index >= 0 ) { 
	    		...
	    		if ( lngFecha + 1 < lngHoy ) {
	    			Dialog.inform("No se pueden registrar visitas anteriores a ayer.");
	    			//Estilos.pushScreen(new IngresoDatosCI(canalIndustrial, inf, obra));
	    			return;
	    		}
    ...
    public void drawListRow(ListField list, Graphics g, int index, int y, int w) {
    	if ( list == lstTitulo ) {
            ...
            g.clear();
            g.drawText("Marca Observacion", 0, y, DrawStyle.LEFT, w);
    	}
    	if ( list == lstTomas ) {
