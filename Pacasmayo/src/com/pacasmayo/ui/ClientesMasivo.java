...
import com.pacasmayo.dao.CanalMasivoDB;
...
public class ClientesMasivo extends MainScreen implements ListFieldCallback, FieldChangeListener {        
	...
	private CanalMasivoDB canales = new CanalMasivoDB();
    ...
    private void seleccion() {
    	...
    	if( index >= 0 ) {
    		CanalMasivo cm = get(index);
    		Estilos.pushScreen(new InformacionMasivo(cm, txtFechaDesde.getDate(), txtFechaHasta.getDate()));
    	}
    }            
       
    public ClientesMasivo() {
...
//	        }
    ...
    public CanalMasivo get(int index) {
        return (CanalMasivo) canales.getObjetos().elementAt(index); // opciones[index];
    }
